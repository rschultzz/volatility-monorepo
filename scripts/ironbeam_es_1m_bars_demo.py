...
def main():
    ...
    # -----------------------------------------------------------------------
    # After WS closes: build DataFrame, filter to today's bars, plot
    # -----------------------------------------------------------------------
    if not all_bars:
    ...
    print(f"Bars for today (UTC {today_utc}): {len(df_today)}")

    if df_today.empty:
        print("No bars for today's UTC date (might be running outside RTH or before data exists).")
        return
    ...
    df_plot = df_today[["open", "high", "low", "close"]].copy()
    ...
    mpf.plot(
        ...
        title="ES front month – 1m OHLC (today UTC)",
        ylabel="Price",
        datetime_format="%H:%M",
        tight_layout=True,
        volume=False,       # flip to True once you wire volume in cleanly
    )
