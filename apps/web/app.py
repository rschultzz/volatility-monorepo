...
def _load_es_minutes_with_features(
    eng,
        ...
        [
            ...
            html.Div(
                [
                    ...
                    html.Div([make_skew_block(), make_term_metrics_block()]),
                ],
                style={
                    "display": "grid",
...
def _backtests_tab(default_start: dt.date, default_end: dt.date) -> html.Div:
    # Defaults taken from scripts/run_gex_fade_backtest.py
    return html.Div(
        [
            html.Div(
                [
                    html.Div(
                        [
                            ...
                            html.Div(
                                "Run gex_fade on es_minutes_with_features and inspect every trade.",
            ...
            html.Div(
                [
                    ...
                    _num_input("Min |put skew| at entry (pp)", BT_MIN_ABS_SKEW_ID, 0.5, step=0.1, min_=0),
...
def serve_layout():
    ...
    # Backtest defaults: last ~60 calendar days ending today (or Fri if weekend)
    bt_end = default_trade_date
    bt_start = bt_end - dt.timedelta(days=60)

    return html.Div(
        [
            # Keep these always-present so Ironbeam/smile callbacks don't break when switching tabs
            ...
            dcc.Tabs(
                ...
                children=[
                    dcc.Tab(
                        label="Dashboard",
                        value=TAB_DASHBOARD,
                        children=_dashboard_tab(default_trade_date),
                    ...
                    dcc.Tab(
                        label="Backtests",
                        value=TAB_BACKTESTS,
                        children=_backtests_tab(bt_start, bt_end),
                        style={"backgroundColor": "black", "color": "#e5e7eb", "border": "1px solid #1f2937"},
                        selected_style={"backgroundColor": "#0b0f19", "color": "white", "border": "1px solid #1f2937"},
                    ),
                ],
...
def run_backtest_cb(
    ...
    eng = _engine()
    if eng is None:
        msg = "DATABASE_URL is not set (set it in Render env vars or local .env)."
        return [], [], [], msg, None
    ...
    status = f"Running backtest on {len(df):,} rows…"
    try:
    ...
    # Make table columns stable + readable
    preferred_cols = [
        "trade_date",
        "entry_ts_pt",
        "entry_bar_index",
