...
- With the re-backfilled 5/20 row (range_pts=300), the recomputed `walls` include the **+7520 structural magnet** (previously missed at range_pts=200). This is the smoke test that the range_pts bump did its job — verified: the stored 5/20 walls now hold `+7520.6 (+945B)` alongside the two negative walls at 7253.6 and 7342.6.

  **Amendment — endpoint smoke test:** the original criterion also expected the recomputed `peaks_by_bucket['30+ DTE']` to contain a peak around 7520–7530, and the curl check below expected a confluence with "30+ DTE participating". Real-data smoke testing shows neither holds on the stored grid, and the reason is structural, not a bug. The 30+ DTE bucket's GEX on 5/20 is a broad **plateau** — ≈381B at 7520, declining only to ≈367B at the stored grid's 7657.6 upper edge — so `scipy.signal.find_peaks` cannot establish the prominence drop it needs on the rising side, and returns no 30+ DTE peak. The Phase 0 script *does* surface a 30+ DTE peak (its top confluence prints `7506 ★★★ (00d, 1-7, 30+)`), but only because the script re-centers its grid on the analytical spot (7392 → window out to 7692), gaining ~34pt of upper headroom that the **table_spot-centered stored grid** (centered 7357.6 → window to 7657.6) does not have. The endpoint reads the spot-agnostic stored landscape and cannot re-center — that is the deliberate ADR tradeoff. The headline magnet is still fully delivered: it is present in the recomputed `walls`, in the `total` landscape curve the panel renders, and in the 8-30 DTE bucket (which peaks ≈7530). The endpoint's 5/20 confluences are therefore `≈7456` and `≈7505`, both 2-bucket (0DTE + 1-7 DTE) — which matches the Phase 0 reference's `★★` grade for both. Closing the 30+ DTE bucket-peak gap would require `range_pts > 300` (Out of Scope) or a change to the stored grid's centering (architectural, Out of Scope). Filed as a known limitation; revisit alongside the deferred `range_pts` tuning.
...
- Hit the endpoint directly via curl: `curl "http://localhost:8060/api/gex-landscape?ticker=SPX&date=2026-05-20&spot=7392&implied_move=40" | jq .`. Confirm shape matches spec; eyeball regime is `magnet-above`, drift_target ≈7520, the `+7520` magnet is present in `walls`, neg zone at ≈7343, and confluences at ≈7456 and ≈7505 (both 2-bucket: 0DTE + 1-7 DTE — see the Acceptance Criteria amendment on why 30+ DTE does not join). The `+7520` magnet in `walls` is the proof the range_pts bump did its job.
- Compare against script: capture script stdout for the same params (with `--range-pts 300`); verify the classifier outputs match. Note (per the Acceptance Criteria amendment): `regime`, `walls`, `neg_zones`, `intraday_subtarget`, `bucket_summary` dominance, and per-bucket regimes match the script; the **confluence list differs** — the script's grid is centered on the analytical spot while the endpoint's stored grid is centered on `table_spot`, so peaks near the upper grid edge (the 30+ DTE plateau) cluster differently. This divergence is expected and documented, not a defect.
