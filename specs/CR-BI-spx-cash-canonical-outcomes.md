...
⚑ n < 20. **Reading (one look, no re-cuts):** in this frame neither test separates the walls from a level at the same IM-scaled distance on any other day. Touch: +1.5 / +2.2 pts, CIs ± 8–11 pts, centred near zero — a high raw touch rate (68 % / 89 %) is what a level ≈ 0.8 IM above the open gets anyway over 5 / 20 sessions in a rising market. Containment: close-inside is exactly at placebo; range-inside is +2.6 pooled with the CI spanning zero (amplification +6.9 is the only cell whose interval nearly excludes zero — one of twelve cells). What this does **not** show: the tests are powered only for effects ≳ 8–10 pts (touch) / 5 pts (containment); the frame is the strike-axis conversion of a landscape whose kernel centres are themselves mis-placed (A), so a wall located correctly might behave differently; and these tests say nothing directly about the debit's P&L, which is a close-path number, not a touch number (note that its `beat` compares the edge-gated fill to a first-minute fill *on the same signal dates* — it measures the entry gate, not the signal; a no-signal baseline for the P&L does not exist yet).

## Step 7 — promotion (own commit, last code change)

`packages/shared/canonical_version.CANONICAL_FEATURE_VERSION = "v0.6.1-nyse-sessions"`. Read-only check with the constant promoted: `cr_b` targets (feature rows without an outcome at canonical) = 0, so the 13:40 cron will not re-insert history; the sweep sees the 5 pending rows (2026-08-18, 08-27, 09-03, 09-04, 09-14) and will promote them on NYSE sessions; `bt_daily_outcomes_active` / `bt_daily_features_active` hold 815 / 815 rows at the new version; `compute_structural_probability` defaults to it; the Proposals card spot row resolves (2026-09-18 → 7710.0, unchanged — still the ES open). No hardcoded `v0.6.0-openiv` string exists in `apps/`, `packages/` or the cron scripts.

## Wrap (2026-09-20)

- **Delivered:** shared session definition + NYSE-calendar sessions everywhere outcomes read bars; `cr_g` / `cr_i` UTC windows retired; `v0.6.1-nyse-sessions` (run `a44f185c`) promoted; harness settles on SPX cash with AM-settled exclusion and a quote cross-check; restated reference `CR-BI` (run `9b292f22`); follow-up Step 0 material (A, B). Outcomes remain ES-vs-B. `v0.6.0-openiv`, `v0.6.0-openiv-spxcash` and the `CR-AR` cells are untouched.
- **Deltas vs A1:** none in scope. Noted: the harness re-selects its threshold on train, so the persisted `CR-BI` debit cells are at T = 0.10; the fixed-T 0.05 numbers are in Step 5.
- **Merge procedure (Ryan):** (1) outside 10:00–13:50 UTC; (2) **immediately before merging, re-run `apps/web/.venv/bin/python scripts/cr_bi_backfill_nyse_sessions.py`** — it is idempotent (`ON CONFLICT DO NOTHING`) and copies the feature / outcome rows the still-deployed crons have written under `v0.6.0-openiv` since run `a44f185c` (every trading day until the merge adds one); (3) merge with a merge commit — `sweep_pending_outcomes`, `backfill_outcomes`, `open-implied-move`, `orats-eod-gamma` auto-deploy from `main`; (4) **manual redeploys: `Dash - MonoRepo` (web — reads the constant), `daily-leg-capture` (reads the constant), `reference-rerun` (new harness settlement; first `REF-2026-09` run on 2026-10-01 will put the SPX-cash reference on the v2 card).**
- **Behaviour changes on deploy:** analogue outcomes on both cards come from `v0.6.1` (12 touch/close labels differ; tN closes and ≈ 30–40 post-touch labels per timeframe differ, mostly PST-month rows); horizons of pending rows are counted in NYSE sessions; nothing else. The card's reference block keeps showing `CR-AR` until the October `REF-` run.
- **Deferred / unfiled:** ES RTH bars missing on 2023-12-15 and 2025-12-19; in-sample threshold selection in the harness; `detect_touch` still ES-vs-B; everything frame-related (follow-up CR: `compute_discounted_level` direction and day count, target frame, placebo results).
