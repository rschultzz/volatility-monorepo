"""SPX-cash session series from 0DTE `orats_monies_minute` rows (CR-BH per-period hybrid series).

Spec: specs/CR-BH-spx-cash-shadow-outcomes.md, Step 0 Q7 + Amendment A1. Moved here by CR-BI
(specs/CR-BI-spx-cash-canonical-outcomes.md, A1.4) so the edge harness can settle on it.
...
settlement_prints(clean) -> Series[date -> last print 12:50–13:00 PT]   (CR-BI harness settlement)
...
def fetch_minutes(conn, date_from: dt.date, date_to: dt.date) -> pd.DataFrame:
    """Raw per-minute spot / stock prints, [date_from, date_to] inclusive, session window only."""
...
def settlement_prints(clean: pd.DataFrame) -> pd.Series:
    """Last clean print in 12:50–13:00 PT per session date (the harness's settlement underlying).

    On lagged 2023 days that is the 13:00 close print (median stock_price 12:56–13:00).
    Dates with no print in the window (early closes, outages) are absent.
    """
    if clean.empty:
        return pd.Series(dtype=float)
    t = clean["minute"].dt.time
    w = clean[(t >= SETTLE_START) & (t <= SESSION_END)].sort_values("minute")
    return w.groupby("session_date")["px"].last()
