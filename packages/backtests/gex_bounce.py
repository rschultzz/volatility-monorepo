...
@dataclass
class GexBounceParams:
    """
    Parameters for the 'buy into lower GEX support using put skew collapse' strategy.
    This is the mirror of the GEX fade (short) logic.
    """

    # How close price has to be to be considered "near" the support wall (clusters/entries)
    entry_proximity_max: float = 0.75

    # How close price has to be to be considered a true "touch" of the support wall
    ...
    # Minimum *decrease* in put skew (as a fraction of baseline) between baseline (Test 1)
    # and later test. 0.7 means "put skew collapsed by at least 70%".
    put_skew_decrease_min: float = 0.7
    ...
    gex_wall_min: float = 1e11  # |gex_wall_below_gex| must be at least this big
    ...
    # Require entries to be in the *bottom* portion of the day's RTH range.
    # 0.0 disables the filter; 0.3 means "only buy if price is in the bottom 30%".
    entry_max_range_frac: float = 0.3

    # Optional: for longs, target the next big GEX wall *above* price
    # instead of a fixed R-multiple.
    use_upper_gex_target: bool = False
    upper_gex_min_abs: float = 1e11  # min |gex_wall_above_gex| to trust as a real level


def run_gex_bounce_backtest(
    df_features: pd.DataFrame,
    params: Optional[GexBounceParams] = None,
    start_date: Optional[pd.Timestamp] = None,
    end_date: Optional[pd.Timestamp] = None,
) -> Tuple[pd.DataFrame, Dict[str, Any]]:
    """
    Run the 'buy lower GEX support' backtest on es_minute_features data.
    """
    if params is None:
...
def _run_day(day: pd.DataFrame, trade_date, params: GexBounceParams) -> List[Dict[str, Any]]:
    """
    Run a single trading day for the long-support strategy.
    """
...
def _build_tests_for_day(day_rth: pd.DataFrame, params: GexBounceParams) -> pd.DataFrame:
    """
    Identify tests of the LOWER GEX wall (support) for a single day.

    - Tests are clusters of bars that get *near* the support wall
      (dist_to_wall_below_pts <= entry_proximity_max).
    ...
    - For each test we store:
        ...
        * touch_ts_utc / touch_bar_index  -> first true tag of the wall (if any),
                                             defined by dist_to_wall_below_pts <= touch_proximity_max
    ...
    # "Near wall" condition for clustering on the LOWER wall
    cond = (
        day["gex_wall_below"].notna()
        & day["gex_wall_below_gex"].abs().ge(params.gex_wall_min)
        & day["net_gex"].gt(params.gex_net_min)
        & day["dist_to_wall_below_pts"].between(0, params.entry_proximity_max)
    )
    ...
    # Group near-wall bars by rounded wall price (support levels)
    nw["wall_key"] = np.round(nw["gex_wall_below"]).astype("float")
    ...
    for wall_key, g in nw.groupby("wall_key"):
        ...
        for j, (_, gt) in enumerate(g.groupby("local_test_id"), start=1):
            ...
            touch_mask = gt["dist_to_wall_below_pts"].le(params.touch_proximity_max)
            if touch_mask.any():
...
def _compute_candidates_for_day(
    ...
    """
    Decide which tests become LONG entry candidates.

    For each wall_key (support) in this day:

    - Baseline = first test with |skew| >= min_baseline_skew.
    - For every *later* test:
        * compute skew change vs baseline,
        * if skew has COLLAPSED enough: change_pct <= -put_skew_decrease_min,
          AND test has a touch_ts_utc,
          create a long entry candidate at that touch time.

    So you get at most ONE candidate per test, and only for tests where:
      - put skew is meaningfully lower (puts cheaper) than baseline, and
      - price actually tagged the support wall in that cluster.
    """
    if tests_df.empty:
        return {}
    ...
    for wall_key, wtests in tests_df.groupby("wall_key"):
        ...
        # ---- For each later test, see if skew has collapsed enough ----
        for idx in range(baseline_idx + 1, len(wtests)):
            ...
            # Fractional change in skew vs baseline
            change_pct = (curr_skew - base_skew) / abs(base_skew) if base_skew != 0 else -np.inf

            # For a bounce, we want skew to be *lower* than baseline by X%
            if change_pct > -params.put_skew_decrease_min:
                continue
            ...
            # Double-check regime and near-wall condition at the touch bar (support)
            gex_wall_below = entry_row.get("gex_wall_below")
            gex_wall_below_gex = entry_row.get("gex_wall_below_gex", 0.0)
            net_gex = entry_row.get("net_gex", 0.0)
            dist_to_wall = entry_row.get("dist_to_wall_below_pts", np.inf)

            if (
                pd.isna(gex_wall_below)
                or round(float(gex_wall_below)) != round(float(wall_key))
                or abs(float(gex_wall_below_gex)) < params.gex_wall_min
                or float(net_gex) <= params.gex_net_min
                or not (0 <= float(dist_to_wall) <= params.entry_proximity_max)
            ):
                continue

            # Build candidate (LONG)
            cand = {
                "wall_key": float(wall_key),
                "entry_ts": entry_ts,
                ...
                "put_skew_entry_ts_pt": entry_ts_pt,
            }

            # If multiple candidates share the same entry_ts (e.g. overlapping walls),
            # keep the one with the largest magnitude skew collapse.
...
def _simulate_day(
    ...
    """
    Given intraday bars and entry candidates, simulate LONG trades for one day.
    """
    trades: List[Dict[str, Any]] = []
    ...
    # Day's RTH range for "bottom of range" filtering
    ...
    for _, row in day.iterrows():
        ...
        # --- Manage existing position first (LONG) ---
        if position is not None:
            ...
            if stop_hit and target_hit:
                exit_price = target_price  # assume best-case
            ...
            if exit_trade:
                pnl_points = exit_price - position["entry_price"]  # long
                trades.append(
                    {
                        ...
                        "gex_wall_below": position["gex_wall_below"],
                        "gex_wall_below_gex": position["gex_wall_below_gex"],
        ...
        if position is None:
            ...
            # Double-check regime and near-wall condition at the actual bar (support)
            gex_wall_below = row.get("gex_wall_below")
            gex_wall_below_gex = row.get("gex_wall_below_gex", 0.0)
            net_gex = row.get("net_gex", 0.0)
            dist_to_wall = row.get("dist_to_wall_below_pts", np.inf)
            ...
            # Only buy near the *bottom* of the day's range, if requested
            entry_frac = (entry_price - day_low) / day_range  # 0=low, 1=high
            if params.entry_max_range_frac > 0.0 and entry_frac > params.entry_max_range_frac:
                continue

            # --- Risk & target for LONGs ---
            stop_price = entry_price - params.stop_loss_points
            target_price = entry_price + params.r_mult * params.stop_loss_points

            # Optional: override target with upper GEX wall if it's big enough
            if params.use_upper_gex_target:
                wall_above = row.get("gex_wall_above")
                wall_above_gex = row.get("gex_wall_above_gex", 0.0)

                if (
                    wall_above is not None
                    and not pd.isna(wall_above)
                    and float(wall_above) > entry_price  # above us
                    and abs(float(wall_above_gex)) >= params.upper_gex_min_abs
                ):
                    target_price = float(wall_above)
            ...
            position = {
                ...
                "gex_wall_below": float(gex_wall_below),
                "gex_wall_below_gex": float(gex_wall_below_gex),
...
def _summarize_trades(trades_df: pd.DataFrame, params: GexBounceParams) -> Dict[str, Any]:
    """
